...
//				// 비회원 메뉴
//				if (Main.login == null) {
//					String[][] menus = { { "로그인", "회원가입", "아이디 찾기", "비밀번호 찾기" }, {}, {} }; // 비회원용 메뉴
//					Out.menu("회원관리", 4, 55, "이전 메뉴", menus);
//
//					menu: switch (In.getStr("메뉴 선택")) {
//
//					// 로그인
//					case "1":
//						Out.titleMini("로그인");
//						LoginVO loginVO = write(new LoginVO(), "id", "pw");
//
//						// 입력한 아이디, 비밀번호와 일치하는 데이터를 받아옴
//						loginVO = (LoginVO) Execute.run(new MemberLoginServiceImpl(), loginVO);
//						if (loginVO != null) {
//							if (loginVO.getStatus().equals("탈퇴")) {
//								Out.error("탈퇴한 회원입니다.", "서비스 이용을 원하시면 다시 회원가입해 주세요."); // 상태가 '탈퇴'일 경우
//							} else {
//								if (loginVO.getStatus().equals("휴면")) { // 상태가 '휴면'일 경우
//									Out.sys("3개월 이상 접속 이력이 없어 휴면회원으로 전환되었습니다.");
//									Out.sys("휴면 해제를 위해 이름과 전화번호를 입력해 주세요.");
//
//									// LoginService에서 받아온 정보와 비교하여 휴면 해제
//									if (In.getStr("이름").equals(loginVO.getName())
//											& In.getStr("전화번호('-' 포함)").equals(loginVO.getTel())) {
//										loginVO.setStatus("정상");
//										if ((Integer) Execute.run(new MemberWakeUpServiceImpl(), loginVO) == 1) {
//											Out.sys("휴면 해제가 완료되었습니다.");
//										} else {
//											Out.sys("휴면 해제가 정상적으로 완료되지 않았습니다. 다시 시도해 주세요.");
//											Out.sysln("가입 시 입력한 정보가 기억나지 않으시면 관리자 이메일로 문의 바랍니다.(admin@dogrea.com)");
//											break menu;
//										}
//									} else {
//										Out.sysln("회원정보가 일치하지 않아 휴면 상태를 해제할 수 없습니다.");
//										break menu;
//									}
//								}
//								Main.login = loginVO;
//								Out.sysln("정상적으로 로그인되었습니다.");
//							}
//						} else {
//							Out.sys("로그인에 실패했습니다.");
//							Out.sysln("아이디와 비밀번호를 확인해 주세요.");
//						}
//						break;
//
//					// 회원가입
...
//						Out.titleMini("회원가입");
//
//						// 회원가입을 위한 정보를 입력받는 메서드 - 데이터 형식과 관련한 모든 검사 수행하고 LoginVO 반환
//						LoginVO joinVO = join();
//						if (joinVO == null)
//							break;
//						if ((Integer) Execute.run(new MemberJoinServiceImpl(), joinVO) == 1) {
//							Out.sys("회원가입에 성공했습니다.");
//							Out.sysln("로그인 후 이용 바랍니다.");
//						}
//						break;
//
//					// 아이디 찾기
//					case "3":
//						Out.titleMini("아이디 찾기");
//						LoginVO findIdVO = write(new LoginVO(), "name", "tel"); // 이름과 전화번호를 입력받아 DB에서 검색
//						String fid = (String) Execute.run(new MemberFindIdServiceImpl(), findIdVO);
//						if (fid != null) {
//							Out.sys("회원님의 아이디는 " + fid + " 입니다.");
//							Out.sysln("로그인 후 이용 바랍니다.");
//						} else {
//							Out.sys("일치하는 정보가 없습니다.");
//							Out.sysln("가입하지 않으셨다면 먼저 회원가입을 진행해 주세요.");
//						}
//						break;
//
//					// 비밀번호 찾기
//					case "4":
//						Out.titleMini("비밀번호 찾기");
//						LoginVO updatePwVO = write(new LoginVO(), "id", "name", "tel"); // 아이디, 이름, 전화번호, 새로운 비번을 입력받음
//						updatePwVO.setPw(Check.newPw(false));
//
//						// 정보가 모두 일치하면 수정 쿼리가 실행됨
//						if ((Integer) Execute.run(new MemberUpdatePwServiceImpl(), updatePwVO) == 1) {
//							Out.sys("비밀번호가 변경되었습니다.");
//							Out.sysln("로그인 후 이용 바랍니다.");
//						} else {
//							Out.sys("일치하는 정보가 없습니다.");
//							Out.sysln("가입하지 않으셨다면 먼저 회원가입을 진행해 주세요.");
...
//						return;
//					default:
//						Out.sysln("1 ~ 4번 또는 0번만 입력할 수 있습니다. 다시 입력해 주세요.");
//					}
//
//					// 회원용 메뉴
//				} else {
//					String[][] menus = { {}, { "로그아웃", "내 정보 보기", "내 정보 수정", "회원탈퇴" }, // 로그인 후 나오는 메뉴
//							{ "회원리스트", "회원정보보기", "관리자변경", "휴면/탈퇴관리" } }; // 관리자만 보이는 메뉴
//					Out.menu("회원관리", 4, 55, "이전 메뉴", menus);
//
//					menu: switch (In.getStr("메뉴 선택")) {
//
...
//			} catch (Exception e) {
//				e.printStackTrace();
//				Out.error("오류내용 : " + e.getMessage(), "관리자에게 문의 바랍니다.");
//			}
//		}
//
//	}
//
//	// 회원가입 메서드
//	private LoginVO join() throws Exception {
//		for (int i = 0; i < Check.RULES.length; i++) { // 회원약관 출력
//			System.out.println(Check.RULES[i]);
//		}
//
//		for (int i = 0; i < Check.PRIVATES.length; i++) { // 개인정보 처리방침 출력
//			System.out.println(Check.PRIVATES[i]);
//		}
//		System.out.println();
//		rule: while (true) {
//			Out.sys("회원약관과 개인정보 처리방침에 동의하시겠습니까?(y/n)");
//			switch (In.getStr("")) {
//			case "y":
//				break rule;
//			case "n":
//				Out.sysln("약관에 동의하지 않아 회원가입을 취소합니다."); // 동의하지 않으면 진행 취소
//				return null;
//			default:
...
//		}
//
//		LoginVO join = new LoginVO(); // 회원가입을 위한 LoginVO 객체 생성
//		while (true) {
//			String newId = In.getStr("아이디");
//			if (!Check.checkId(newId)) { // 아이디 조건 체크 메서드 - 길이, 영문/숫자 등
//			} else if ((boolean) Execute.run(new MemberIdExistsServiceImpl(), newId)) { // 아이디 중복 체크
//				Out.sys("이미 존재하는 아이디입니다. 다른 아이디를 입력해 주세요.");
//			} else {
//				join.setId(newId); // 생성 가능한 아이디이면 LoginVO 객체에 값을 저장하고 반복문 빠져나옴
//				break;
//			}
//		}
//
//		join.setPw(Check.newPw(true)); // 비밀번호 조건 체크 메서드를 거쳐 비밀번호 세팅
//
//		while (true) {
//			String newName = In.getStr("이름"); // 이름 조건 체크 메서드를 거쳐 이름 세팅
//			if (!Check.checkName(newName)) {
//			} else {
//				join.setName(newName);
//				break;
//			}
//		}
//		String birth = Check.checkAge(); // 날짜 형식 및 나이 체크 메서드
//		if (birth == null) {
//			Out.sysln("회원약관에 따라 만 14세 미만은 가입할 수 없습니다."); // 14세 미만인 경우 진행 취소
//			return null;
//		} else
//			join.setBirth(birth);
//		join = write(join, "gender", "address", "tel", "email"); // 나머지 정보 입력받음
//		return join;
//	}
//
